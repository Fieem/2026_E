...
**********************************************************/
/**
  * @brief  读取电机当前脉冲数（阻塞等待应答，校验地址与功能码）
  * @param  addr  电机ID
  * @return 带符号的脉冲数，超时返回 INT32_MIN
  * @note   rxData[0]=0x30, rxData[1]=符号(0正1负), rxData[2..5]=脉冲绝对值(大端)
  */
int32_t Emm_V5_Get_Pulse(uint8_t addr)
{
  uint32_t timeout = 10000;

  can.rxFrameFlag = 0;
  Emm_V5_Read_Sys_Params(addr, S_CLKC);            // 请求读取 S_CLKC(0x30)

  while (--timeout)
  {
    if (can.rxFrameFlag)
    {
      can.rxFrameFlag = 0;                         // 先清标志，准备接收下一条

      // 校验应答来源：ExtId[15:8] 应等于目标电机地址
      ...
      // 校验功能码：rxData[0] 应等于 S_CLKC(0x30)
      if (can.rxData[0] != 0x30)
      {
        continue;                                  // 功能码不匹配，继续等
      }

      // 解析回包: rxData[1]=符号(0正1负), rxData[2..5]=脉冲绝对值(大端)
      uint32_t pulse = ((uint32_t)can.rxData[2] << 24)
                     | ((uint32_t)can.rxData[3] << 16)
                     | ((uint32_t)can.rxData[4] << 8)
                     |  (uint32_t)can.rxData[5];

      return (can.rxData[1] == 0) ? (int32_t)pulse : -(int32_t)pulse;
    }
    HAL_Delay(1);
  }

  return (int32_t)0x80000000;                      // 超时，INT32_MIN，正常脉冲值不可能为 -2147483648
}

/**
  * @brief  读取电机实时位置（S_CPOS, 0x36），返回编码器位置值，阻塞等待应答
  * @param  addr  电机ID
  * @return 编码器位置值（多圈累加，单位同 Emm_V5_Pos_Control 绝对模式的 clk 参数），
  *         超时返回 0xFFFFFFFF
  * @note   应答格式: rxData[0]=0x36, rxData[1]=符号, rxData[2..5]=位置值(大端), DLC=7
  *         返回值直接用于 Emm_V5_Pos_Control / Emm_V5_MMCL_Pos_Control 的绝对模式
  */
uint32_t Emm_V5_Get_Position(uint8_t addr)
{
  uint32_t timeout = 10000;

  can.rxFrameFlag = 0;
  Emm_V5_Read_Sys_Params(addr, S_CPOS);             // 请求读取 S_CPOS(0x36)
  ...
  while (--timeout)
  {
    if (can.rxFrameFlag)
    {
      ...
      // 拼接位置值: rxData[2..5]（大端），单位同绝对位置模式的 clk
...
/**
  ...
  *         速度 50 RPM，加速度 10
  */
void Move(uint8_t addr, int32_t position)
{
    uint8_t  dir    = (position >= 0) ? 0 : 1;         // 0=CW, 1=CCW
    uint32_t pulses = (uint32_t)(position > 0 ? position : -position);

    Emm_V5_Pos_Control(addr, dir, 50, 10, pulses, true, false);
    //                              vel acc clk    raF=T  snF=F
    //                                 绝对模式   非同步

    /* 更新对应轴的 last_pos */
    if (addr == 1) {
        last_pos_yaw   = position;
    } else if (addr == 2) {
        last_pos_pitch = position;
    }
}
